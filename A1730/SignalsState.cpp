// ---------------------------------------------------------------------------

#pragma hdrstop

#include "SignalsState.h"
#include "A1730.h"
#include "uFRSigPanel.h"
#include "uFunctions.h"
// ---------------------------------------------------------------------------
#pragma package(smart_init)
#pragma resource "*.dfm"
TSignalsStateForm *SignalsStateForm;

// ---------------------------------------------------------------------------
__fastcall TSignalsStateForm::TSignalsStateForm(TComponent* Owner,
	TIniFile* _ini) : TForm(Owner)
{
	ini = _ini;
}
// ---------------------------------------------------------------------------

void __fastcall TSignalsStateForm::FormShow(TObject *Sender)
{
	InOutTimer->Enabled = true;
}
// ---------------------------------------------------------------------------

void __fastcall TSignalsStateForm::InOutTimerTimer(TObject *Sender)
{
	TTimer* t = (TTimer*)Sender;
	t->Enabled = false;
	CheckSignals();
	t->Enabled = true;
}
// ---------------------------------------------------------------------------

void __fastcall TSignalsStateForm::FormClose(TObject *Sender,
	TCloseAction &Action)
{
	InOutTimer->Enabled = false;
}
// ---------------------------------------------------------------------------

void TSignalsStateForm::CheckSignals(void)
{
	for (int i = 0; i < ControlCount; i++)
	{
		TControl* p = Controls[i];
		if (p->ClassName() != "TFRSigPanel")
			continue;
		TFRSigPanel* pp = (TFRSigPanel*)p;
		pp->CheckSignal();
	}
}

void __fastcall TSignalsStateForm::FormHide(TObject *Sender)
{
	InOutTimer->Enabled = false;
}
// ---------------------------------------------------------------------------

void __fastcall TSignalsStateForm::FormCreate(TObject *Sender)
{
	LoadFormPos(this, ini);
	int space = 5;

	int ltop0 = space;
	int lwidth;
	for (int i = 0; i < a1730->SignalCount(); i++)
	{
		CSignal* sig = a1730->GetSignal(i);
		if (!sig->GetIn())
			continue;
		TFRSigPanel *p = new TFRSigPanel(this, sig);
		p->Parent = this;
		AnsiString a = "In";
		a += i;
		p->Name = a;
		p->Top = ltop0;
		p->Left = space;
		lwidth = p->Width;
		ltop0 += p->Height + space;
	}
	int ltop1 = space;
	for (int i = 0; i < a1730->SignalCount(); i++)
	{
		CSignal* sig = a1730->GetSignal(i);
		if (sig->GetIn())
			continue;
		TFRSigPanel *p = new TFRSigPanel(this, sig);
		p->Parent = this;
		AnsiString a = "Out";
		a += i;
		p->Name = a;
		p->Top = ltop1;
		p->Left = space + lwidth + space;
		ltop1 += p->Height + space;
	}
	Width = space + lwidth + space + lwidth + space + (Width - ClientWidth);
	if (ltop0 < ltop1)
		ltop0 = ltop1;
	Height = ltop0 + (Height - ClientHeight);

}
// ---------------------------------------------------------------------------

void __fastcall TSignalsStateForm::FormDestroy(TObject *Sender)
{
	SaveFormPos(this, ini);
}
// ---------------------------------------------------------------------------
